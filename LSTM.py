...
print("Starting Simple LSTM Optimization (PC Mode)...")
# Adjust this path to where the CSV is on your friend's PC!
BASE_PATH = 'data'
FILE_PATH = os.path.join(BASE_PATH, 'final_cleaned_data_energy_multicity.csv')
OUTPUT_DIR = BASE_PATH
TARGET = 'elec_mw'
CHAMPION_PATH = os.path.join(OUTPUT_DIR, "best_champion_lstm_simple.pth")
RETRAINED_PATH = os.path.join(OUTPUT_DIR, "best_retrained_lstm_simple.pth")
PARAM_PATH = os.path.join(OUTPUT_DIR, "best_params_lstm_simple.json")
...
# --- 6. MODEL DEFINITION (SIMPLE LSTM) ---
device = 'cuda' if torch.cuda.is_available() else 'cpu'
...
def objective(trial):
    ...
    # Simple LSTM Model
    model = SimpleLSTM(len(all_cols), hidden_size, num_layers, HORIZON, dropout).to(device)
...
print(f"Running Simple LSTM optimization on {torch.cuda.get_device_name(0)}...")
...
# --- 9. RETRAIN ---
print("\nRetraining Simple LSTM with Best Parameters...")
...
print("\n" + "="*40)
print("** FINAL TUNED SIMPLE LSTM RESULTS **")
